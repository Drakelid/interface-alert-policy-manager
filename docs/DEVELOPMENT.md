...
Message placeholders come from `TemplateContextBuilder`. `SafeTemplateRenderer` supports substitutions plus its own constrained `{{#if placeholder}}`, `{{else}}`, `{{/if}}`, `==`, `!=`, `contains`, `not contains`, and `&&` grammar; it never evaluates PHP or Blade. `&&` combines up to 10 requirements and requires every one to match. For `device_groups`, contains tests exact membership; equality accepts either the complete formatted value or one exact member. It throws on unknown placeholders even in an unselected branch, so a new placeholder must be added to the builder (both `forIncident()` and `forPreview()` go through the same map) and to the README list. `TemplateRenderingTest` asserts that every documented placeholder resolves.
