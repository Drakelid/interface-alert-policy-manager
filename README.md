...
IAPM sends the complete rendered message to the SMS gateway without truncation or rewriting. The Template Preview reports the rendered character count, but the gateway controls its final length, truncation, and segmentation behavior. Built-in templates place the potentially verbose interface description last so device, port, timing, policy, and incident details reach gateways first. Custom templates are sent in exactly the order configured by the user.

The available placeholders are `incident_id`, `severity`, `state`, `hostname`, `sysName`, `display_name`, `device_id`, `device_groups`, `port_id`, `ifName`, `ifDescr`, `ifAlias`, `ifAdminStatus`, `ifOperStatus`, `interface_type`, `location`, `policy_name`, `assignment_source`, `first_seen_at`, `triggered_at`, `recovered_at`, `outage_duration`, `device_url`, `port_url`, `acknowledgement_user`, and `suppression_reason`. `device_groups` is a comma-separated list of every LibreNMS device group containing the device, or an empty string when it has none. For message rendering, `ifAlias` removes decorative `#`, `Bundle to`, and Bundle-Ether/Bundle-Ethernet variants while preserving the useful remainder; the stored LibreNMS port description is never changed. `device_url` and `port_url` are built from the `url_base` setting, which defaults to the application URL. Template preview and real delivery use the same placeholder map, so a template that previews cleanly renders identically when it is sent.
