...
SMS-gateway messages are limited to one carrier SMS segment by default: 160 GSM-7 septets or 70 Unicode UTF-16 units. GSM extension characters such as `^`, `{`, and `€` consume two septets. Longer rendered messages are truncated deterministically and retain the incident id. Set `IAPM_SMS_SINGLE_SEGMENT=false` only if concatenated, separately billed SMS segments are intentional; generic webhook messages continue to use the configured 480-character safety limit.
