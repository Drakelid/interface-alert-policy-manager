...
```bash
sudo -u librenms php artisan iapm:health   # exits 0 when ingestion, scheduler, gateway, backlog, and queue are healthy
```
...
- **Ingestion dead-man's switch.** The checks above all watch what happens *after* a fault arrives; `Receiving alerts from LibreNMS` watches whether faults arrive at all. Silence alone is never the signal — a quiet network is supposed to be quiet — so it reports the disagreement instead: LibreNMS logged new alerts on rules that have delivered to IAPM before, and ingestion did not advance. That is what a rule losing its alert operation, a half-rotated token, or a poller that cannot reach the ingestion URL looks like from inside the plugin. Every refused request (`401`, `413`, `415`, `422`, `503`) is recorded in `storage/logs/iapm.log` as `Ingestion request rejected.` with its reason code, throttled to one line per reason per source per minute.
...
- No incidents at all while LibreNMS shows alerts: `iapm:health` reports `[FAIL] Receiving alerts from LibreNMS`. Nothing was POSTed, so nothing in IAPM is at fault. Grep `storage/logs/iapm.log` for `Ingestion request rejected.` — a hit names the reason; no hit means the request never arrived. Then check the rule's alert operation and transport (see `docs/OPERATIONS.md`), and `curl` the ingestion URL **from every poller**, not just the web host: a distributed install fails one poller at a time, and NAT that will not hairpin blocks exactly the node that also serves the endpoint. A `401` from that curl is the healthy answer — it proves the request reached IAPM.
