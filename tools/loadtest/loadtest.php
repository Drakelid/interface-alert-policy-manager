...
$timeq = function (string $label, string $sql, array $binds = [], int $runs = 3): void {
    try {
    ...
    $best = PHP_FLOAT_MAX;
    for ($i = 0; $i < $runs; $i++) { $t = microtime(true); DB::select($sql, $binds); $best = min($best, (microtime(true) - $t) * 1000); }
...
foreach (DB::select('SELECT state, count(*) c FROM iapm_incidents GROUP BY state') as $r) echo '  '.str_pad($r->state, 14).number_format($r->c)."\n";
...
if (getenv('RUN_COMMANDS')) {
    ...
    foreach (['iapm:reconcile', 'iapm:process-actions'] as $cmd) {
        $t = microtime(true);
        Artisan::call($cmd);
