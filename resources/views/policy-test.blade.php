...
@if($resolution->policy)<div class="alert alert-success">Effective policy: <strong><a href="{{ route('iapm.policies.edit',$resolution->policy) }}">{{ $resolution->policy->name }}</a></strong>. @if($resolution->winner)Selected by the {{ $resolution->winner->assignment_type->value }} <a href="{{ route('iapm.assignments.edit',$resolution->winner) }}">assignment</a>.@else Selected by the configured default-policy setting.@endif</div>@else<div class="alert alert-warning"><strong>No effective policy.</strong> This interface would not notify anyone. Add an <a href="{{ route('iapm.assignments.create') }}">assignment</a> covering it, or a default assignment covering everything unmatched.</div>@endif
