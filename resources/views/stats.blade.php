...
<p class="iapm-hint" style="max-width:70em;">Outage history computed from completed (recovered) outages only, so an ongoing incident does not appear until it clears. <strong>MTTA</strong> is first-seen to triggered; <strong>MTTR</strong> is first-seen to recovered.</p>
...
{{-- P4-1: the same tighter, evenly-wrapping tile grid the Overview uses. --}}
<div class="iapm-tile-grid">
    <div class="panel panel-default"><div class="panel-heading">Outages</div><div class="panel-body"><strong>{{ $metrics['outages'] }}</strong></div></div>
    <div class="panel panel-default"><div class="panel-heading" title="Avg first-seen to triggered">MTTA</div><div class="panel-body"><strong>{{ $fmt($metrics['avg_detect']) }}</strong></div></div>
    <div class="panel panel-default"><div class="panel-heading" title="Avg first-seen to recovered">MTTR</div><div class="panel-body"><strong>{{ $fmt($metrics['avg_duration']) }}</strong></div></div>
    <div class="panel panel-default"><div class="panel-heading">Longest</div><div class="panel-body"><strong>{{ $fmt($metrics['max_duration']) }}</strong></div></div>
    <div class="panel panel-default"><div class="panel-heading">Notifications</div><div class="panel-body"><strong>{{ $metrics['notifications'] }}</strong></div></div>
    <div class="panel panel-default"><div class="panel-heading">Flapping outages</div><div class="panel-body"><strong>{{ $metrics['flapping'] }}</strong></div></div>
</div>

<div class="panel panel-default">
    <div class="panel-heading"><h2 style="font-size:15px;margin:0;">Outages per day</h2></div>
    <div class="panel-body">
        {{-- P4-7: this used the tiny inline sparkline, which drew 1px bars with
             no axes or labels — unreadable on a sparse period. --}}
        @include('iapm::partials.bar-chart',['values'=>$spark,'label'=>'Outages per day','unit'=>'outage'])
    </div>
</div>
