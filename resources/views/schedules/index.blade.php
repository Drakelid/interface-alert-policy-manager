...
@if($schedules->count())
<form id="iapm-bulk-schedules" method="post" action="{{ route('iapm.schedules.bulk-destroy') }}" onsubmit="return confirm('Delete the selected schedules? Any used by policies are skipped.')">@csrf @method('DELETE')
    <button class="btn btn-danger btn-sm" style="margin-bottom:8px;"><i class="fa fa-trash"></i> Delete selected</button>
</form>
<div class="table-responsive"><table class="table table-hover">
<thead><tr><th style="width:2em;"><input type="checkbox" onclick="document.querySelectorAll('.iapm-bulk').forEach(e=>e.checked=this.checked)"></th><th>Name</th><th>Timezone</th><th>Mode</th><th>Status</th><th>Policies</th></tr></thead>
<tbody>@foreach($schedules as $s)<tr>
<td><input class="iapm-bulk" type="checkbox" form="iapm-bulk-schedules" name="ids[]" value="{{ $s->id }}"></td>
