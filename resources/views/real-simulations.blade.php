...
<table class="table table-striped table-condensed">
    ...
    @forelse($simulations as $simulation)
        ...
        <tr>
            ...
            <td><time class="iapm-time">{{ $simulation->started_at?->format(\LibreNMS\Plugins\InterfaceAlertPolicyManager\Support\TimeText::FORMAT) ?? '—' }}</time></td>
            <td>@if($simulation->status === 'running')<time class="iapm-time" datetime="{{ $simulation->recover_at?->toIso8601String() }}">{{ $simulation->recover_at?->format(\LibreNMS\Plugins\InterfaceAlertPolicyManager\Support\TimeText::FORMAT) ?? '—' }}</time> <span class="iapm-hint" data-iapm-countdown="{{ $simulation->recover_at?->toIso8601String() }}">{{ $simulation->recover_at?->diffForHumans() }}</span>@else<time class="iapm-time">{{ $simulation->recovered_at?->format(\LibreNMS\Plugins\InterfaceAlertPolicyManager\Support\TimeText::FORMAT) ?? '—' }}</time>@endif</td>
