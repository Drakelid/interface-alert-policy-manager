...
<p class="iapm-hint iapm-wrap-code" style="max-width:70em;">The default message sent for each event type. A policy action with its own template overrides these. Leave a box blank to use the built-in default, shown as the placeholder text. Only <code>@{{ placeholder }}</code> substitutions run &mdash; no PHP or Blade &mdash; and an unknown placeholder is rejected on save rather than sent as literal text. Try wording on the <a href="{{ route('iapm.template-preview') }}" target="_blank">Template Preview</a> page.</p>
