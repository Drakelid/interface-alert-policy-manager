...
<div class="panel {{ $values['has_token'] ? 'panel-default' : 'panel-warning' }}" id="ingestion-token">
    ...
    <div class="panel-body">
        ...
        <form method="post" action="{{ route('iapm.settings.rotate-token') }}" data-iapm-confirm="Rotate ingestion token? The previous token stays valid for 15 minutes.">@csrf
...
<div class="panel panel-default" id="policy-defaults">
    ...
    <div class="panel-body">
        <div class="form-group"><label for="iapm-set-default_policy_id">Default policy</label>
            <select name="default_policy_id" class="form-control" id="iapm-set-default_policy_id"><option value="">None</option>@foreach($policies as $p)<option value="{{ $p->id }}" @selected($values['default_policy_id']==$p->id)>{{ $p->name }}</option>@endforeach</select>
            <p class="iapm-hint">Used when no assignment matches an interface. A default <em>assignment</em> is usually preferable.</p>
        </div>
        <div class="form-group"><label for="iapm-set-sms_default_receiver">Global SMS receiver</label>
            <input class="form-control" name="sms_default_receiver" id="iapm-set-sms_default_receiver" value="{{ old('sms_default_receiver',$values['sms_default_receiver']) }}">
            <p class="iapm-hint">Last-resort receiver when a policy, assignment, or destination does not specify one.</p>
        </div>
        <div class="form-group"><label for="iapm-set-url_base">URL base</label>
            <input class="form-control" name="url_base" id="iapm-set-url_base" value="{{ old('url_base',$values['url_base']) }}" placeholder="https://librenms.example.com">
            <p class="iapm-hint">Used to build device/port links inside notification messages.</p>
        </div>
    </div>
</div>
...
<div class="panel panel-default" id="delivery-retention">
    ...
    <div class="panel-body">
        <div class="form-group"><label for="iapm-set-notification_timeout">Notification timeout (seconds)</label><input class="form-control" name="notification_timeout" id="iapm-set-notification_timeout" value="{{ old('notification_timeout',$values['notification_timeout']) }}"><p class="iapm-hint">How long to wait for the gateway before recording a failure.</p></div>
        <div class="form-group"><label for="iapm-set-notification_retry_count">Notification retries</label><input class="form-control" name="notification_retry_count" id="iapm-set-notification_retry_count" value="{{ old('notification_retry_count',$values['notification_retry_count']) }}"><p class="iapm-hint">Extra attempts per delivery when the gateway errors (0–10).</p></div>
        <div class="form-group"><label for="iapm-set-retention_days">Retention (days)</label><input class="form-control" name="retention_days" id="iapm-set-retention_days" value="{{ old('retention_days',$values['retention_days']) }}"><p class="iapm-hint">Recovered incidents, events, delivery and audit logs older than this are cleaned up daily. Active incidents are never deleted.</p></div>
        <input type="hidden" name="record_unpoliced" value="0">
        <div class="checkbox"><label for="iapm-set-record_unpoliced"><input type="checkbox" id="iapm-set-record_unpoliced" name="record_unpoliced" value="1" @checked(old('record_unpoliced',$values['record_unpoliced']))> <strong>Record alerts for interfaces with no policy</strong></label>
            <p class="iapm-hint">On (default): un-matched interfaces are stored as suppressed <code>no_policy</code> incidents for visibility. <strong>Turn off on very large fleets</strong> that intentionally scope IAPM to specific interfaces — alerts for everything else are then ignored instead of creating hundreds of thousands of rows.</p>
        </div>
        <div class="form-group"><label for="iapm-set-deleted_port_behavior">Deleted-port behaviour</label>
            <select class="form-control" name="deleted_port_behavior" id="iapm-set-deleted_port_behavior"><option value="recover" @selected($values['deleted_port_behavior']==='recover')>Recover incident when the port is deleted</option><option value="retain" @selected($values['deleted_port_behavior']==='retain')>Retain the open incident</option></select>
        </div>
    </div>
</div>
...
<div class="panel panel-default" id="delivery-dispatch">
    ...
    <div class="panel-body">
        <div class="form-group"><label for="iapm-set-dispatch_mode">Dispatch mode</label>
            <select class="form-control" name="dispatch_mode" id="iapm-set-dispatch_mode">
...
<div class="panel panel-default" id="storm-control">
    ...
    <div class="panel-body">
        <div class="form-group"><label for="iapm-set-aggregate_threshold">Aggregate threshold</label>
            <input class="form-control" name="aggregate_threshold" id="iapm-set-aggregate_threshold" value="{{ old('aggregate_threshold',$values['aggregate_threshold']) }}">
            <p class="iapm-hint">When this many interfaces on the <strong>same device</strong> go down together, send one grouped "device down" SMS instead of one per interface. Set <code>0</code> to disable (always notify per interface). Recommended: 3.</p>
        </div>
        <div class="form-group"><label for="iapm-set-aggregate_window_seconds">Aggregation window (seconds)</label>
            <input class="form-control" name="aggregate_window_seconds" id="iapm-set-aggregate_window_seconds" value="{{ old('aggregate_window_seconds',$values['aggregate_window_seconds']) }}">
            <p class="iapm-hint">Interfaces that trigger within this window of each other are grouped. Default 120.</p>
        </div>
        <p class="iapm-hint">Customise the digest wording under <a href="{{ route('iapm.message-templates') }}">Message Templates</a>.</p>
    </div>
</div>
...
<div class="panel panel-default" id="root-cause">
    ...
    <div class="panel-body">
        <div class="form-group"><label for="iapm-set-uplink_port_group_id">Uplink port group</label>
            <select class="form-control" name="uplink_port_group_id" id="iapm-set-uplink_port_group_id"><option value="">None</option>@foreach($portGroups as $g)<option value="{{ $g->id }}" @selected($values['uplink_port_group_id']==$g->id)>{{ $g->name }}</option>@endforeach</select>
            <p class="iapm-hint">Ports in this LibreNMS port group are treated as uplinks. When an uplink on a device is down, policies with "suppress when uplink down" will suppress the downstream customer interfaces on that device, avoiding an alert storm.</p>
        </div>
    </div>
</div>
