@extends('layouts.librenmsv1') @section('title','IAPM Synthetic Simulation') @section('content')
<div class="container-fluid">@include('iapm::partials.nav')
<h1 class="iapm-page-title">Synthetic simulation</h1>
...
<p class="iapm-hint" style="max-width:70em;">Inject a synthetic observation for one interface to validate policy, assignment, and suppression behaviour. Incidents are created and updated as normal; external delivery still respects dry-run mode. For a timed test that changes LibreNMS port state, runs actions and automatically recovers, use <a href="{{ route('iapm.real-simulations.index') }}">Real Simulation</a>. To evaluate policy without creating anything, use <a href="{{ route('iapm.policy-test') }}">Policy Test</a>.</p>
