...
<p class="iapm-hint iapm-wrap-code" style="max-width:70em;">Render a message template against one real interface to see exactly what would be sent after the active @can('manage iapm settings')<a href="{{ route('iapm.sms-content-filters.edit') }}">SMS Content Filters</a>@else SMS Content Filters @endcan. Nothing is delivered. Insert values with <code>@{{ placeholder }}</code>, or conditionally include text with <code>@{{#if ifAlias}}...@{{else}}...@{{/if}}</code> and comparisons such as <code>@{{#if severity == "critical"}}...@{{/if}}</code>. No PHP or Blade runs; invalid syntax and unknown placeholders are reported.</p>
