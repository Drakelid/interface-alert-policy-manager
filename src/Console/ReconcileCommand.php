...
class ReconcileCommand extends Command
{
    ...
    public function handle(InterfaceContextService $contexts, PolicyResolver $resolver, SuppressionService $suppression, SettingStore $settings): int
    {
        ...
        $changed = 0; $failed = 0;
        $query->chunkById((int) config('iapm.processing.batch_size', 500), function ($items) use (&$changed, &$failed, $contexts, $resolver, $suppression, $settings): void {
            ...
            foreach ($items as $incident) {
                try {
                    ...
                    $reason = $suppression->reason($policy, $context, ! (bool) $port->device->status, SuppressionService::maintenanceSuppresses($port->device), SuppressionService::anyParentDown($port->device->parents));
    ...
    private function transition(Incident $incident, IncidentState $state, string $message, array $attributes = []): void { if ($this->option('dry-run')) { $this->line("Would set incident {$incident->id} to {$state->value}: $message"); return; } $incident->update(array_merge($attributes, ['state' => $state])); $incident->events()->create(['event_type' => 'reconciled', 'event_message' => $message, 'event_data' => ['state' => $state->value]]); }
}
