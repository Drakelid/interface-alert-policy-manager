...
class CleanupCommand extends Command
{
    ...
    public function handle(SettingStore $settings): int
    {
        ...
        $this->table(['Record type', 'Eligible'], [['Recovered incidents', $incidentCount], ['Incident events', $eventCount], ['Delivery logs', $deliveryCount], ['Audit logs', $auditCount]]);
        if (! $this->option('force')) { $this->info('Dry-run only. Re-run with --force to delete eligible records.'); return self::SUCCESS; }
        DB::transaction(function () use ($recovered, $cutoff): void { DB::table('iapm_audit_logs')->where('created_at', '<', $cutoff)->delete(); $recovered->chunkById(500, fn ($incidents) => Incident::whereKey($incidents->modelKeys())->delete()); });
