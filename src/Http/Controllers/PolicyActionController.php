...
class PolicyActionController extends Controller
{
    ...
    public function store(Request $r, Policy $policy, AuditService $audit)
    {
        abort_unless($r->user()->can('manage iapm policies'), 403);
        $a = $policy->actions()->create($this->validated($r));
    ...
    public function update(Request $r, PolicyAction $action, AuditService $audit)
    {
        abort_unless($r->user()->can('manage iapm policies'), 403);
        $before = $action->toArray();
        $action->update($this->validated($r));
