...
class IngestionController extends Controller
{
    public function __invoke(IngestAlertRequest $request, StateNormalizer $states, InterfaceContextService $contexts, PolicyResolver $policies, SuppressionService $suppression, DependencyResolver $dependencies, ReceiverResolver $receivers, IncidentLifecycleService $lifecycle): JsonResponse
    {
        ...
        foreach ($data['faults'] as $fault) {
            ...
            if (! $resolution->policy) {
                ...
                $this->runWithUniqueRetry(function () use ($data, $state, $context, $fingerprint, &$counts, $lifecycle): void {
                    ...
                    if ($state === 'recovered') {
                        ...
                        if ($incident->exists && $lifecycle->recover($incident, 'Recovery received from LibreNMS without an effective policy.')) {
                            $counts['recovered']++;
                        } else {
                            $counts['ignored']++;
                        }
            ...
            $this->runWithUniqueRetry(function () use ($data, $state, $context, $resolution, $suppression, $dependencies, $device, $fingerprint, &$counts, $receivers, $lifecycle): void {
                ...
                if ($state === 'recovered') {
                    if ($incident && $incident->state !== IncidentState::Recovered) {
                        $this->requestRecovery($incident, $resolution->policy->recovery_after_seconds, 'Recovery received from LibreNMS.', $counts);
                    }
        ...
        if (isset($data['alert_id']) || isset($data['alert_uid']) || isset($data['rule_id'])) {
            $query = Incident::where('device_id', $device->device_id)->where('state', '!=', IncidentState::Recovered)->where(function ($query) use ($data): void {
                if (isset($data['alert_id'])) {
                    $query->orWhere('source_alert_id', $data['alert_id']);
                } if (isset($data['alert_uid'])) {
                    $query->orWhere('source_alert_uid', (string) $data['alert_uid']);
                } if (! isset($data['alert_id']) && ! isset($data['alert_uid']) && isset($data['rule_id'])) {
                    $query->orWhere('source_rule_id', $data['rule_id']);
                }
            });
            if ($state === 'recovered') {
                $seen = [];
            ...
            $query->with('policy')->when($seen !== [], fn ($q) => $q->whereNotIn('port_id', $seen))->chunkById((int) config('iapm.processing.batch_size', 500), function ($incidents) use (&$counts): void {
                DB::transaction(function () use ($incidents, &$counts): void {
                    foreach ($incidents as $incident) {
                        $this->requestRecovery($incident, (int) ($incident->policy?->recovery_after_seconds ?? 0), 'Interface disappeared from current fault set.', $counts);
                    }
                });
            });
        }
        app(SettingStore::class)->put('last_ingestion_at', now()->toIso8601String());
        Log::channel('iapm')->info('Alert ingestion completed.', ['device_id' => $device->device_id, 'alert_id' => $data['alert_id'] ?? null, 'alert_uid' => $data['alert_uid'] ?? null, 'state' => $state, 'counts' => $counts]);
    ...
    {
        if ($holdDown <= 0) {
            if (app(IncidentLifecycleService::class)->recover($incident, $message)) {
                $counts['recovered']++;
            } else {
                $counts['ignored']++;
            }
